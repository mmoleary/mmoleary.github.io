...
```{r}
dat.kitsberg <- read.csv("Fig.3B-Table 1.csv")
dat.kitsberg.cleaned = dat.kitsberg |> separate("sample", 
...
dat.kitsberg.cleaned.summary <- dat.kitsberg.cleaned |>
  group_by(SampleType) |> #seperates data by sample category
...
labs(x = "Age in Years", # Specify axis labels
     y = "Price (Millions of USD)") +
ggtitle("Price based on Years") # Plot title
```
...
@tbl-PriceAgeCorrelation shows that there is a moderately negative relationship between age and price. It is likely monotonic rather than linear as the Pearson correlation is not as strong as the Spearman correlation. But this is not diffinitive as the Kendall Tau is slighlty less strong than Pearson.
